...
describe("ExecutionFold — Output-widget areas (RISKS #17)", () => {
  const claim = (id: string, msgId = "req-1"): [string, any] =>
    ["comm_msg", { comm_id: id, data: { method: "update", state: { msg_id: msgId } } }];
  const release = (id: string): [string, any] => claim(id, "");
  const png = (aid: string): [string, any] =>
    ["display_data", { data: { "image/png": { $tithon_artifact: { artifact_id: aid } } }, metadata: {} }];
  ...
  it("a widget's clear spares its sibling outputs", () => {
    const msgs: Array<[string, any]> = [
      ["display_data", { data: { "application/vnd.jupyter.widget-view+json": { model_id: "bar" } }, metadata: {} }],
    ];
    for (let i = 0; i < 4; i++) {
  ...
  it("a cell-level clear still clears everything", () => {
    expect(foldMessages([stream("t"), claim("out"), png("a"), release("out"),
      ["clear_output", { wait: false }]])).toEqual([]);
  });

  it("a synthetic cell-scope clear overrides an active claim", () => {
    expect(foldMessages([stream("t"), claim("out"), png("a"),
      ["clear_output", { wait: false, tithon_scope: "cell" }]])).toEqual([]);
  });
  ...
  it("nested claims restore the outer area", () => {
    const out = foldMessages([claim("a"), claim("b"), release("b"), png("x"),
      ["clear_output", { wait: false }], stream("still here")]);
    expect(out.map((o) => o.output_type)).toEqual(["stream"]);
  });

  it("comm_close releases a claim left open", () => {
    expect(foldMessages([stream("t"), claim("out"), ["comm_close", { comm_id: "out" }],
      ["clear_output", { wait: false }]])).toEqual([]);
  });
  ...
  it("state() round-trips through seed() so a reconnect keeps scoping", () => {
    ...
    for (const m of [claim("out"), ["clear_output", { wait: true }] as [string, any], png("f0")]) a.apply(m[0], m[1]);

    const b = new ExecutionFold();
    b.seed(a.outputs(), a.state());
    for (const m of [["clear_output", { wait: true }] as [string, any], png("f1"), release("out")]) b.apply(m[0], m[1]);

    const direct = new ExecutionFold();
    for (const m of [claim("out"), ["clear_output", { wait: true }] as [string, any], png("f0"),
      ["clear_output", { wait: true }] as [string, any], png("f1"), release("out")]) direct.apply(m[0], m[1]);
