  ...
  return cell.outputs
    .map((o, i) => {
      const items = o.items.map((it) => {
        const head =
          it.mime === WIDGET_MIME || it.mime.startsWith("text/") || it.mime.endsWith("+json")
...
describe("baseline.py training loop (tqdm x3 + Output-widget plot + \\r print)", function () {
  ...
  it("runs cells 0..36 and reports what the loop cell actually renders", async function () {
    ...
    await waitFor(
      () => nb.getCells().some((c) => c.outputs.length > 0),
      120000,
    ...
    const ticker = setInterval(() => {
      ...
      console.log(
        `[trainloop] progress: cells-with-output=${done} loopOutputs=${nb.cellAt(LOOP_CELL).outputs.length}`,
      );
      reveal();
    }, 15000);
    ...
    console.log(
      `[trainloop] SUMMARY widgets=${mimes.filter((m) => m === WIDGET_MIME).length}` +
