...
describe("Tithon cell interrupt + re-run (v24)", () => {
  it("the stop button interrupts the cell and the kernel survives for re-run", async () => {
    ...
    await runCell(uri, 0);
    await waitFor(
      () => cellText(nb.cellAt(0)).includes("RUN 1") && maxTick(cellText(nb.cellAt(0))) >= 3,
      30000,
      "loop running",
    );
    ...
    await waitFor(
      ...
      20000,
      "cell interrupted (success=false)",
    );
    ...
    assert.ok(
      after - stoppedAt <= 1,
      `loop kept running after interrupt: ${stoppedAt} -> ${after}`,
    );
    ...
    await runCell(uri, 0);
    await waitFor(
      () => cellText(nb.cellAt(0)).includes("RUN 2"),
      20000,
      "re-run after interrupt (RUN 2)",
    );
