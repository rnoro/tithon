...
function outputsWith(
  ...
  pred: (it: vscode.NotebookCellOutputItem) => boolean,
): number {
...
function textIncludes(it: vscode.NotebookCellOutputItem, needle: string): boolean {
  return (
    (it.mime.includes("stdout") || it.mime.includes("stderr") || it.mime === "text/plain") &&
...
describe("Tithon rich outputs in a real VSCode host (v28)", () => {
  it("renders matplotlib PNG + tqdm, and restores the widget final-state text", async () => {
    ...
    await waitFor(
      () => imageBytes(nb.cellAt(mplIdx)) !== undefined,
      45000,
      "matplotlib image to render",
    );
    ...
    assert.ok(
      !streamText(nb.cellAt(mplIdx)).includes("<Figure"),
    ...
    await waitFor(
      () => streamText(nb.cellAt(tqdmIdx)).includes("100%"),
      30000,
      "tqdm to reach 100%",
    );
    ...
    await waitFor(
      () => nb.cellAt(nbIdx).executionSummary?.timing?.endTime !== undefined,
      30000,
    ...
    console.log(
      `[v28] combo cell after restore: ${combo.outputs.length} outputs ` +
        `(image=${imgOutputs}, "combo stdout line"=${stdoutLine}, "after combo plot"=${afterPlot})`,
    );
    assert.ok(
      combo.outputs.length >= 3,
