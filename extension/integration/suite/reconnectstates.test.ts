  ...
  it("done / running / queued cells are all restored", async () => {
    ...
    const code = (kw: string) => {
      const i = cells.findIndex(
        (c) => c.kind === "code" && c.body.some((l) => l.text.includes(kw)),
      );
      assert.ok(i >= 0, `fixture missing ${kw}`);
      return { i, src: cellSource(cells[i]) };
    };
    const A = code("DONE_CELL"),
      B = code("range(40)"),
      C = code("QUEUED_CELL");
    ...
    for (const x of [A, B, C]) {
      await driver.execute(x.src, {
        uri: uri.toString(),
        range: { start: 0, end: 0 },
        cell_hash: computeCellHash(x.src),
        index: x.i,
      });
    }
    ...
    await w.attach(0);
    const status = (src: string) =>
      w.executions().find((e) => e.cellHash === computeCellHash(src))?.status;
    ...
    await waitFor(
      () =>
        status(A.src) === "done" &&
        status(B.src) === "running" &&
        /(^|\n)3(\n|$)/.test(outOf(B.src)),
      30000,
      "A done, B running (>=3 lines), C queued",
    );
    ...
    console.log(
      `[v16] after reconnect: A.out=${JSON.stringify(cellText(nb.cellAt(A.i)))} A.summary=${summ(A.i)}`,
    );
    console.log(
      `[v16] B.out=${JSON.stringify(cellText(nb.cellAt(B.i)).split("\n").filter(Boolean).slice(-3).join(","))} B.summary=${summ(B.i)}`,
    );
    ...
    assert.notStrictEqual(
      nb.cellAt(B.i).executionSummary?.success,
      true,
      "B is still running, not done",
    );
