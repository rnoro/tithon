...
describe("REGRESSION H1: Run All stops at the first cell error", () => {
  it("a middle cell raising skips the later cells (native Run All semantics)", async () => {
    ...
    console.log(
      `[H1] cell0=${JSON.stringify(t0.trim())} cell1=${JSON.stringify(t1.trim().slice(0, 40))} cell2=${JSON.stringify(t2.trim())} cell2Summary=${nb.cellAt(2).executionSummary?.success}`,
    );
    ...
    console.log(
      `[H1] FINDING: after the error in cell 1, cell 2 ${cell2Ran ? "DID run (REGRESSION — stop-on-error broken)" : "did NOT run (native Run All; skipped)"}`,
    );
    // Stop-on-error: cell 2 must be skipped — no output and no completed execution.
    assert.strictEqual(
      cell2Ran,
      false,
      "later cells must be SKIPPED after an error (native Run All)",
    );
    assert.strictEqual(
      nb.cellAt(2).executionSummary?.success,
      undefined,
      "skipped cell must not show a completed run",
    );
  });
});
