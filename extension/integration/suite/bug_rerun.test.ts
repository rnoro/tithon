...
describe("BUG H2: re-run + GENUINE reconnect shows only the latest run", () => {
  it("after running a cell twice and opening fresh, the cell shows VALUE 2 once", async () => {
    ...
    const CODE = cellSource(
      parse(readFileSync(uri.fsPath, "utf8")).cells.filter((c) => c.kind === "code")[0],
    );
    const driver = new SessionClient(undefined, uri.toString());
    await driver.attach(0);
    const h = computeCellHash(CODE);
    const e1 = await driver.execute(CODE, {
      uri: uri.toString(),
      range: { start: 0, end: 2 },
      cell_hash: h,
      index: 0,
    });
    const e2 = await driver.execute(CODE, {
      uri: uri.toString(),
      range: { start: 0, end: 2 },
      cell_hash: h,
      index: 0,
    ...
    console.log(
      `[H2] FINDING: restored=${JSON.stringify(t)} | 'VALUE 2' x${count(t, "VALUE 2")} | stale 'VALUE 1'=${t.includes("VALUE 1")}`,
    );
