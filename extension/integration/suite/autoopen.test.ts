...
describe("Tithon auto-restore + live on open, no command (v22)", () => {
  it("opening a file with a running loop shows output and keeps streaming", async () => {
    ...
    await waitFor(
      () => lines(cellText(nb.cellAt(loopIdx))).length >= 4,
      20000,
      "auto-restored prior output",
    );
    ...
    await waitFor(
      () => Math.max(...lines(cellText(nb.cellAt(loopIdx))), -1) > restoredMax,
      25000,
      "live output continued after open",
    );
