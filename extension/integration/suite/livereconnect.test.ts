...
describe("Tithon DEMO: disconnect + reconnect keeps state & streaming", () => {
  it("restores output+state and continues streaming after a reconnect", async () => {
    ...
    const loopIdx = cells.findIndex(
      (c) => c.kind === "code" && c.body.some((l) => l.text.includes("range(120)")),
    );
    ...
    await waitFor(
      () => maxLine(cellText(nb.cellAt(loopIdx))) >= 3,
      30000,
      "streaming before disconnect",
    );
    ...
    await waitFor(
      () => maxLine(cellText(nb.cellAt(loopIdx))) >= beforeDrop,
      30000,
      "prior output restored on reconnect",
    );
    ...
    await waitFor(
      () => maxLine(cellText(nb.cellAt(loopIdx))) > atReconnect + 2,
      30000,
      "live continued after reconnect",
    );
    ...
    console.log(
      `[demo] streaming continued after reconnect, now at step ${maxLine(cellText(nb.cellAt(loopIdx)))}`,
    );
