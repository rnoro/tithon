...
describe("BUG H5: empty / comment-only cell completes cleanly", () => {
  it("a comment-only cell and a truly-empty cell both finish (no stuck spinner)", async () => {
    ...
    await waitFor(
      ...
      25000,
      "comment-only cell completes",
    );
    const ok0 = nb.cellAt(0).executionSummary?.success;
    console.log(
      `[H5] comment-only cell success=${ok0}, output=${JSON.stringify(cellText(nb.cellAt(0)).trim())}`,
    );
    assert.strictEqual(
      ok0,
      true,
      "comment-only cell should complete successfully (no stuck spinner)",
    );
    ...
    await waitFor(
      () => nb.cellAt(emptyIdx).executionSummary?.success !== undefined,
      25000,
      "empty cell completes",
    );
    const okE = nb.cellAt(emptyIdx).executionSummary?.success;
    console.log(
      `[H5] FINDING: empty cell success=${okE}, activeExecCells=${JSON.stringify(await vscode.commands.executeCommand("tithon._activeExecCells"))}`,
    );
    assert.strictEqual(
      okE,
      true,
      "a truly-empty cell should complete successfully (no stuck spinner)",
    );
