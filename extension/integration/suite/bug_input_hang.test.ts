...
describe("REGRESSION H3: input() bridges to an input box and the cell continues", () => {
  it("answers input() via the input box, the cell completes, and a later cell runs", async () => {
    ...
    try {
      ...
      await runCell(uri, 0);
      await waitFor(
        () => nb.cellAt(0).executionSummary?.success !== undefined,
        30000,
        "input() cell completes",
      );
      const cell0 = cellText(nb.cellAt(0));
      console.log(
        `[H3] input() cell success=${nb.cellAt(0).executionSummary?.success} prompt=${JSON.stringify(seenPrompt)} output=${JSON.stringify(cell0.trim().slice(0, 120))}`,
      );
      assert.ok(
        seenPrompt && /name/.test(seenPrompt),
        `the input box should show the cell's prompt; got ${JSON.stringify(seenPrompt)}`,
      );
      assert.strictEqual(
        nb.cellAt(0).executionSummary?.success,
        true,
        "input() cell should COMPLETE via the bridge, not error/hang",
      );
      assert.ok(
        /GOT Ada/.test(cell0),
        `the answered value should be bound; got ${JSON.stringify(cell0)}`,
      );
    } finally {
    ...
    assert.ok(
      cellText(nb.cellAt(1)).includes("AFTER_INPUT"),
      "a later cell must run after input() was answered",
    );
  });
});
