...
### 🚧 아직 spike인 것 — VSCode 실구동 (Phase 1 UX)

`extension/`의 **로직은 검증됐지만, 실제 VSCode 안에서의 구동은 아직**이다:

- ✅ **구독→fold→복원→부착 경로 구현·검증됨**: `sessionClient.ts`(attach+folding) +
  `outputFold.ts`(folding.py의 TS 포트) + `cellAttach.ts`(cell_hash 부착). **실제 데몬에 셀을
  제출·실행한 뒤 fresh 클라이언트로 재접속하면 folded 출력이 복원되어 셀에 붙는다**(v7).
- ❌ `.vsix` 패키징·VSCode 설치·실제 구동 **안 됨**. `sessionController.ts`(NotebookController로
  복원 출력을 셀에 쓰는 바인딩)는 **컴파일만 되는 spike** — 실 VSCode 호스트에서 미구동.
- ❌ 라이브 스트리밍(실행 *중* 출력을 셀에 실시간 반영)은 미구현 — 현재는 재접속 시 1회 복원.

> **결론(갱신):** "VSCode tunnel로 붙으면 셀 출력이 그대로 유지되는가?"
> — **영속화 + 클라이언트 복원 로직은 완성·검증**됐다. 재접속 시 출력을 folding해 셀에 부착하는
> 절반이 이제 존재하며 실데몬으로 증명된다(v7). **남은 것은 그 로직을 실제 VSCode 확장으로
> 패키징해 호스트 안에서 구동**하는 것(`sessionController` 스파이크를 .vsix로). 즉 "tunnel 재접속
> → 출력 복원"은 헤드리스로는 성립하고, VSCode UI 구동만 Phase 1에 남았다.
...
```bash
make verify        # v1~v7 전체 (현재 7/7 PASS)
...
- v1~v6: Phase 0 종료 조건(설계 가설 증명). v7: Phase 1 첫 조각(클라이언트 복원 절반).
