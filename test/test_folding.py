...
class TestExecutionFold:
    ...
    def test_clear_output_immediate(self):
        out = fold_messages(
            [stream("stdout", "x"), ("clear_output", {"wait": False}), stream("stdout", "y")]
        )
    ...
    def test_execute_result_and_error(self):
        out = fold_messages(
            [
                (
                    "execute_result",
                    {"data": {"text/plain": "42"}, "metadata": {}, "execution_count": 3},
                ),
...
def claim(comm_id, msg_id="req-1"):
    ...
    return (
        "comm_msg",
        {"comm_id": comm_id, "data": {"method": "update", "state": {"msg_id": msg_id}}},
    )
    ...
    def test_widget_clear_spares_sibling_outputs(self):
        ...
        msgs = [
            (
                "display_data",
                {
                    "data": {"application/vnd.jupyter.widget-view+json": {"model_id": "bar"}},
                    "metadata": {},
                },
            )
        ]
        ...
        assert (
            out[1]["data"]["image/png"]["$tithon_artifact"]["artifact_id"] == "f3"
        )  # only latest frame
    ...
    def test_synthetic_cell_scope_overrides_an_active_claim(self):
        ...
        out = fold_messages(
            [
                stream("stdout", "cell text"),
                claim("out"),
                png("a"),  # claim deliberately never released
