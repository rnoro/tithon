...
> **명명**: 그리스 신화의 티토노스(Tithonos)에서. 새벽의 여신 에오스는 제우스에게 연인
> 티토노스의 불사(不死)는 얻어냈지만 영원한 젊음을 청하는 것을 잊었고, 티토노스는 죽지
> 못한 채 영원히 시들어가다 매미가 되었다. Tithon 사용자도 마찬가지다: 영속하는 REPL은
> 얻었지만(불사), 꾸준한 연구 개발 없이는 그 가치가 시들 뿐이다(젊음의 부재). 그 저주를
> 뒤집는 것이 Tithon의 존재 이유다 — _immortality, with eternal youth this time._
>
> 매미(영원히 노래하는 존재)를 로고 모티프로 삼는다: 끊김 없이 출력을 스트리밍하는 세션의
> 메타포다. ti-**thon**의 어미가 Py**thon**과 호응하는 것은 덤이다.
>
> PyPI/npm 네임스페이스 가용 확인 완료 (2026-06). 패키지/CLI: `tithon`
> (`pip install tithon`).
